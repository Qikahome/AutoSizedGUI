...
/**
 * 可配置槽数的测试容器（NeoForge / Fabric 共用，dev-only）。
 * 槽数经菜单打开时的 extra data（vanilla openMenu 的 buf writer / Fabric
 * ExtendedScreenHandlerType）从服务端传给客户端，因此注册单个 MenuType 即可覆盖任意档位。
 * MenuType 由调用方（各 loader 注册处）注入，本类不依赖任何 loader。
 */
public class TestContainer extends AbstractContainerMenu {
    ...
    public TestContainer(MenuType<? extends TestContainer> type, int id, Inventory playerInventory, int slotCount) {
        ...
        // Player inventory: 主背包(27)在前、快捷栏(9)在后

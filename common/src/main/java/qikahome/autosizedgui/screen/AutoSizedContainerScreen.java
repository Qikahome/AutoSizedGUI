...
public class AutoSizedContainerScreen<T extends AbstractContainerMenu> extends AbstractContainerScreen<T> {
    ...
    protected void addBackground() {
        panel.setBackground(new NinePatchRenderer(
                AutoSizedGUIConstants.BUILT_IN_GUI_TEXTURE,
                0, 0,
                15, 15,
                256, 128,
                7, 7, 7, 7));
    ...
    public boolean hasClickedOutside(double mouseX, double mouseY, int guiLeft, int guiTop, int mouseButton) {
        // getGuiLeft()/getGuiTop() are loader extensions (NeoForge), not present in vanilla —
        // use the inherited leftPos/topPos fields (mirrored from the panel via setOrigin).
        int left = leftPos;
        int top = topPos;

...
/**
 * 布局引擎在运行时读取的配置值（纯 Java，无 loader 依赖）。
 * <p>
 * 各 loader 的 config（NeoForge ModConfigSpec / Fabric JSON config）在加载与重载时
 * 把当前值同步到本类字段，common 代码只与本类交互。默认值与两个 loader 的 config 默认一致。
 */

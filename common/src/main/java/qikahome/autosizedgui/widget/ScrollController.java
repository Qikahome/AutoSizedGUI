...
public class ScrollController {
    ...
    public void render(GuiGraphicsExtractor gui, int mouseX, int mouseY) {
        ...
        // Top fixed 1px
        gui.blit(RenderPipelines.GUI_TEXTURED, AutoSizedGUIConstants.BUILT_IN_GUI_TEXTURE,
                trackL, viewportTop,
                TRACK_U, TRACK_V,
    ...
    public void tick(double mouseX, double mouseY, long now) {
        ...
        trackPressY = mouseY;
        if (!trackRepeatArmed) {
            if (now - trackRepeatStart >= AutoSizedGUISettings.scrollbarTrackRepeatDelay) {
                trackRepeatArmed = true;
                trackRepeatStart = now;
            }
            return;
        }
        if (now - trackRepeatStart < AutoSizedGUISettings.scrollbarTrackRepeatInterval)
            return;

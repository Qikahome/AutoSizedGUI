 ...
 * <p>
 * Draws a GUI background consisting of four fixed-size corners, four
 * stretchable edges,
 * and a solid-color center fill &mdash; textured from a source region in the
 * given texture.
...
public class NinePatchRenderer {
     ...
     * @param textureFileWidth  actual pixel width of the PNG file (for UV
     *                          normalisation)
    ...
    /**
     * Sample the center fill color from the texture pixel at ({@code texU + left},
     * {@code texV + top}).
    ...
    private void sampleCenterColor() {
        ...
        try {
            ...
            if (res != null) {
                ...
                centerColor = image.getPixel(sampleX, sampleY);
                image.close();
            }
     ...
     */
    public void render(GuiGraphicsExtractor g, int x, int y, int width, int height) {
        if (!centerColorSampled) {
            sampleCenterColor();
        }
        ...
        // ---- Corners (no stretch) ----
        // top-left
        g.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, texU, texV, left, top, left, top, tw, th);
        // top-right
        g.blit(RenderPipelines.GUI_TEXTURED, texture, x + width - right, y,
                texU + texWidth - right, texV, right, top, right, top, tw, th);
        // bottom-left
        g.blit(RenderPipelines.GUI_TEXTURED, texture, x, y + height - bottom,
                texU, texV + texHeight - bottom, left, bottom, left, bottom, tw, th);
        // bottom-right
        g.blit(RenderPipelines.GUI_TEXTURED, texture, x + width - right, y + height - bottom,
                texU + texWidth - right, texV + texHeight - bottom, right, bottom, right, bottom, tw, th);

        // ---- Edges (stretched to fill remaining space) ----
        // top edge
        if (innerW > 0) {
            g.blit(RenderPipelines.GUI_TEXTURED, texture, x + left, y,
                    texU + left, texV, innerW, top, srcInnerW, top, tw, th);
        }
        // bottom edge
        if (innerW > 0) {
            g.blit(RenderPipelines.GUI_TEXTURED, texture, x + left, y + height - bottom,
                    texU + left, texV + texHeight - bottom, innerW, bottom, srcInnerW, bottom, tw, th);
        }
        // left edge
        if (innerH > 0) {
            g.blit(RenderPipelines.GUI_TEXTURED, texture, x, y + top,
                    texU, texV + top, left, innerH, left, srcInnerH, tw, th);
        }
        // right edge
        if (innerH > 0) {
            g.blit(RenderPipelines.GUI_TEXTURED, texture, x + width - right, y + top,
                    texU + texWidth - right, texV + top, right, innerH, right, srcInnerH, tw, th);
        }
